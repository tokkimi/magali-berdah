...
import { Users, Send, CheckCircle, XCircle, TrendingUp, Star } from 'lucide-react'
import Link from 'next/link'
import Image from 'next/image'

const OR = '#F37021'

export default function ProjetCollaborations() {
  const myCollabs = mockCollaborations.filter(c => c.fromId === 'demo_proj' || c.fromId === 'project-1')
    ...
    <div style={{ color: 'white', maxWidth: 720 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem', marginTop: 2 }}>Influenceurs et ambassadeurs pour votre projet</p>
        </div>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        {myCollabs.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '2.5rem', background: 'rgba(255,255,255,0.02)', borderRadius: 14, border: '1px dashed rgba(255,255,255,0.1)' }}>
            ...
            <p style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.3)' }}>Explorez la galerie pour proposer des collaborations à des influenceurs !</p>
          </div>
        ) : myCollabs.map(c => (
