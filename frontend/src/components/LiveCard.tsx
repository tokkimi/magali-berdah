...
export default function LiveCard({ live }: { live: WhatnotLive }) {
  const isVideo = !!live.previewUrl && /\.(mp4|webm|ogg)(\?|$)/i.test(live.previewUrl);
  return (
    <a href={live.whatnotUrl} target="_blank" rel="noreferrer"
      ...
      <div style={{ aspectRatio: '16 / 10', position: 'relative', background: 'linear-gradient(135deg,#2b201c,#050505)' }}>
        {isVideo ? (
          <video src={live.previewUrl} autoPlay muted loop playsInline style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ) : live.previewUrl ? (
          <img src={live.previewUrl} alt={`Live de @${live.username}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ) : (
      ...
      <div style={{ padding: '12px 14px', display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
        <div style={{ minWidth: 0 }}>
          <p style={{ color: 'white', font: '600 .86rem Helvetica Neue, Arial', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>@{live.username} est en live</p>
          {live.title && <p style={{ color: '#aaa', font: '.7rem Helvetica Neue, Arial', marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{live.title}</p>}
        </div>
